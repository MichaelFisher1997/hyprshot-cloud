...
/// Delete an S3 bucket
pub fn deleteBucket(allocator: std.mem.Allocator, bucket: []const u8, config: Config) !void {
    ...
    // For deleting a bucket, the URI is /{bucket}
    ...
    // Empty payload for bucket deletion
        ...
        if (run_result.term.Exited == 0) {
            ...
            if (run_result.stdout.len > 0) {
                ...
                if (std.mem.indexOf(u8, run_result.stdout, "<Error>") != null) {
                    std.debug.print("Bucket deletion failed: {s}\n", .{run_result.stdout});
                    return error.BucketDeletionFailed;
                }
            }
            std.debug.print("Successfully deleted bucket: {s}\n", .{bucket});
        } else {
            std.debug.print("Bucket deletion failed with exit code: {d}\n", .{run_result.term.Exited});
