...
/// Create an S3 bucket
pub fn createBucket(allocator: std.mem.Allocator, bucket: []const u8, config: Config) !void {
    ...
    // For creating a bucket, the URI is /{bucket}
    ...
    const url = try std.fmt.allocPrint(allocator, "{s}/{s}", .{ config.endpoint, bucket });
    ...
    if (result) |run_result| {
        ...
        if (run_result.term.Exited == 0) {
            ...
            // S3 returns 200 OK for successful bucket creation
            if (run_result.stdout.len > 0) {
                // Check for error in response
                if (std.mem.indexOf(u8, run_result.stdout, "<Error>") != null) {
                    std.debug.print("Bucket creation failed: {s}\n", .{run_result.stdout});
                    return error.BucketCreationFailed;
                }
            }
            std.debug.print("Successfully created bucket: {s}\n", .{bucket});
        } else {
            std.debug.print("Bucket creation failed with exit code: {d}\n", .{run_result.term.Exited});
